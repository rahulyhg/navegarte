...
namespace App\Middlewares {
    ...
    {
        /**
         * Registra middleware para verificação do token
         *
        ...
        public function __invoke(Request $request, Response $response, callable $next)
        {
            try {
                ...
                if ($type === 'Basic' && $token !== env('API_TOKEN', env('API_TOKEN_BASIC', null))) {
                ...
                if ($type === 'Bearer' && !$payload = $this->encryption->decrypt($token)) {
                    if ($token !== env('API_TOKEN', env('API_TOKEN_BASIC', null))) {
